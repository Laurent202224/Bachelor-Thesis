...
extern bool start_OA_algorithm;

extern uint32_t counter111;
extern float velx111;
...
bool fly_task(FlyCommand_t fly_command)
{
    ...
    if (last_time == 0)
    {
        if (!start_OA_algorithm) { //only start counting the time once the OA algorithm has started
            // delta_time = 0;
            last_time = 0;
        }
        else {
            DEBUG_PRINT("flying time starts\n");
            last_time = get_time_stamp();
        }
    }
    uint32_t delta =  get_time_stamp();
    if (!start_OA_algorithm) {
        delta = 0;
    }
    if(
    ...
    {
        DEBUG_PRINT("time flown: %d\n", delta-last_time);
        DEBUG_PRINT("max flight: %d\n", ToFly * MAX_FLIGHT_DURATION);
        DEBUG_PRINT("avg_vel: %f\n", velx111/(float)counter111); //DIST
        DEBUG_PRINT("counter: %f\n", (float)counter111); //DIST
        DEBUG_PRINT("time flow f: %f\n", (float)(delta-last_time)/1000.0); //DIST
        float distancess = (velx111/(float)counter111)*(float)(delta-last_time)/1000.0; //DIST
        DEBUG_PRINT("Dist: %fm\n", distancess); //calculates the total distance flown during the complete flight until the drone landed //DIST
        Land();
        DEBUG_PRINT("Landed\n"); 
        isFlying = false;
        return isFlying;
    ...
    DEBUG_PRINT("Dist: %f", (velx111/(float)counter111)*(float)(delta-last_time)/1000.0);
