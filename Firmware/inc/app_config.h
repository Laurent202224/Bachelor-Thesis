 ...
 * Copyright (c) 2020-2022 InvenSense Inc. All rights reserved.
 *
 ...
 * These values are only used when INCLUDE_ALGO_EXTERNAL is NOT set.
 */
...
#define CHIRP_ICU30201_MAX_RANGE_MM     (10000)  /* maximum range, in mm for ICU30201 */
 ...
 */
#define	CHIRP_STATIC_REJECT_SAMPLES	(0)	/* static target rejection sample
 ...
 * in main.c, which is passed to the ch_meas_init() function.
 */
// #define CHIRP_THRESH_0_START	(0)       /* threshold 0 - must start at sample zero */
// #define CHIRP_THRESH_0_LEVEL	(1500)		//2500

// #define CHIRP_THRESH_1_START	(340)      /* threshold 1 */ //40
...
#define CHIRP_THRESH_0_START	(0)       /* threshold 0 - must start at sample zero */
#define CHIRP_THRESH_0_LEVEL	(6000)		//2500

#define CHIRP_THRESH_1_START	(0)      /* threshold 1 */ //40
#define CHIRP_THRESH_1_LEVEL	(0)

#define CHIRP_THRESH_2_START	(0)      /* threshold 2 */
#define CHIRP_THRESH_2_LEVEL	(0)

#define CHIRP_THRESH_3_START	(0)     /* threshold 3 */
#define CHIRP_THRESH_3_LEVEL	(0)

#define CHIRP_THRESH_4_START	(0)     /* threshold 4 */
#define CHIRP_THRESH_4_LEVEL	(0)

#define CHIRP_THRESH_5_START	(0)     /* threshold 5 */
#define CHIRP_THRESH_5_LEVEL	(0)

#define CHIRP_THRESH_6_START	(0)     /* threshold 6 */
#define CHIRP_THRESH_6_LEVEL	(0)
 ...
 */
#define IMPORT_MEASUREMENT	 		(1)		/* if non-zero, measurement will be imported */

/* Configuration of measurement segments
 * The following symbols define the individual transmit and receive segments within
 * the measurement.
 */
#define CHIRP_TX_SEG_CYCLES         (512)	/* Transmit segment - length (cycles) */
...
#define CHIRP_RX_SEG_1_SAMPLES      (337)	/* Receive segment 1 - remainder of measurement */
 ...
 */
#define CHIRP_RINGDOWN_FILTER_SAMPLES (20)		/* number of samples to filter */  //TODO: was at 20 initially
 ...
 *
 *   To use a different sensor firmware type (for example, a new distribution
 *   from TDK Invensense), simply define CHIRP_SENSOR_FW_INIT_FUNC to equal the name
 *   of the init routine for the new firmware.
 * 
 ...
 * 
 *      * Change CHIRP_SENSOR_FW_INIT_FUNC to icu_init_init
 *      * Change APP_DATA_MAX_SAMPLES to 680
 ...
 *      * Add a define to your build: -DINCLUDUE_ALGO_EXTERNAL
 *        - MPLABX:
 *          + Right-click project -> Properties
 *          + Under desired build config (e.g., Release) expand ARM GCC
 *          + Under ARM GCC click arm-gcc
 *          + Change "option categories" to "preprocessing and messages"
 *          + Click on "defined symbols"
 *          + Add INCLUDE_ALGO_EXTERNAL
 *      * Delete the algo config from measurement_config.c
 *      * If using IMPORT_MEASUREMENT, ensure your measurement queue has no more
 *        than 680 RX samples total
 */

#define CHIRP_SENSOR_FW_INIT_FUNC	(icu_gpt_init) /* ICU GPT - general purpose transceiver */
//#define CHIRP_SENSOR_FW_INIT_FUNC	(icu_init_init) /* ICU INIT - no rangefinding; increased max IQ samples */
...
#define DISPLAY_MULTI_TARGET	(0)		/* if non-zero, multiple detected targets displayed
